...
(function() {
    ...
    function toggleHide()
    {
        ...
        let cp = document.getElementById("cb_content_panel")
    ...
    function injectContentPanel()
    {
        document.getElementById("cb_content_panel")?.remove()
        document.body.prepend(contentPanel);
    }

    function analyseGameLog()
    {
        try{
    ...
    function extractInfoFromPost(post, gameLogs, currentIndex) {
        ...
        if (contents.length > 0) {
            ...
            // Actions
            if( contents.length > 1 ) // at least 1 action
            {
                let text = $(contents[1]).text()
    ...
    function changeResourcesFromPlayer(playername, resource, amount, fromBank = true, otherPlayerName="") {
        ...
        if( !resource ) // if resources should be sent to another player instead of bank and its undefined.
            ...
            {
                case "brick" :
                    game.players[indexOfPlayer].resources.brick += amount;
                    fromBank? game.bank.brick -= amount : game.players[indexOfOtherPlayer].resources.brick -= amount;
                    break;
                case "ore" :
                    game.players[indexOfPlayer].resources.ore += amount;
                    fromBank? game.bank.ore -= amount : game.players[indexOfOtherPlayer].resources.ore -= amount;
                    break;
                case "grain" :
                    game.players[indexOfPlayer].resources.grain += amount;
                    fromBank? game.bank.grain -= amount : game.players[indexOfOtherPlayer].resources.grain -= amount;
                    break;
                case "lumber" :
                    game.players[indexOfPlayer].resources.lumber += amount;
                    fromBank? game.bank.lumber -= amount : game.players[indexOfOtherPlayer].resources.lumber -= amount;
                    break;
                case "wool" :
